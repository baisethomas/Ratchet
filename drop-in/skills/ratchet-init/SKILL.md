...
- The lint hook maps file types to linters in `lint_command_for` (ESLint, ruff, swiftlint, gofmt, rustfmt, rubocop, shellcheck by default). Delete the rows this repo does not use. **Only the ESLint row is verified by the template's tests**; run each remaining row once against the installed version before trusting it, and fix the flags if they differ. A mapped file type whose linter is not installed is reported on every edit rather than silently skipped, so either install the tool or remove its row — do not leave a row that will nag. File types with no row are not linted; if this repo's main language has no row, add one, and if no linter exists for it, say so in the report.
