...
- **Procedures load themselves.** A prompt that has to be pasted depends on someone remembering it. `drop-in/skills/` turns the recurring procedures into skills the agent loads when the task matches: the completion gate (`ratchet-done`) and test-first bug fixing with a script that proves the test can fail (`ratchet-bugfix`). Rules stay in `AGENTS.md`, which always loads; a skill carries only the how.
