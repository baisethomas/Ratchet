...
- **CODEX.md is a Codex adapter, and carries the routing table.** Use `drop-in/CODEX.md` when Codex is part of the workflow. It states what changes without hooks and routes delegated work by blast radius, not apparent size: reviews go up a tier, a second failed verification escalates, and verification is identical at every tier.
