...
- **CODEX.md is a Codex adapter.** Use `drop-in/CODEX.md` when Codex is part of the workflow. It states what changes when the hooks are not around the tool calls, and where Codex finds skills.
- **Route delegated work by blast radius, not apparent size.** `AGENTS.md` defines the tiers — orchestrate, operate (high risk and routine), git and docs — with what each owns and never does; each adapter only names the model that fills a tier. Reviews go up a tier, a second failed verification escalates, and verification is identical at every tier.
- **Procedures load themselves.** A prompt that has to be pasted depends on someone remembering it. `drop-in/skills/` turns the recurring procedures into skills the agent loads when the task matches: the completion gate (`ratchet-done`), test-first bug fixing with a script that proves the test can fail (`ratchet-bugfix`), hostile review where every finding is reproduced before it is acted on (`ratchet-review`), the handoff (`ratchet-handoff`), and the ratchet itself — one permanent fix per escaped failure, a check before a rule (`ratchet-tighten`). `ratchet-init` installs the lot. Rules stay in `AGENTS.md`, which always loads; a skill carries only the how. Several of these procedures borrow from Matt Pocock's skills; see `drop-in/skills/CREDITS.md`.
- **Ship the fixed part as a plugin.** The skills and the destructive-command guard are the same in every repo, so they install once, from the repo's own marketplace, and update in one command; what must differ per repo (the contract's repo specifics, project memory, the stop gate's check command) stays a drop-in. Copies drift; a plugin does not.
